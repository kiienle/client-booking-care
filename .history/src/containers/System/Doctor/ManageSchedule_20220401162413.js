...
class ManageSchedule extends Component {
    ...
    render() {
        console.log("kien le check state: ", this.props);
        const { scheduleHoursRedux, language } = this.props;
        return (
            <React.Fragment>
                    ...
                    <div className="container">
                        <div className="row">
                            ...
                            <div className="col-12 pick-hour-container">
                                {scheduleHoursRedux.map((item, i) => (
                                    <div
                                        key={item.id}
                                        className="schedule-hours-list"
                                    >
                                        <a className="schedule-hours-item">
                                            {language === LANGUAGES.VI
                                                ? item.valueVi
                                                : item.valueEn}
                                        </a>
                                        <a
                                            className="schedule-hours-item"
                                            key={item.id}
                                        ></a>
                                    </div>
                                ))}
...
const mapStateToProps = (state) => {
    return {
        ...
        scheduleHoursRedux: state.admin.scheduleHours,
    };
};
